...
// Inicializa a variável $subcategorias como um array vazio.
...
try {
    // Prepara a consulta SQL para selecionar todas as subcategorias da tabela 'subcategoria'.
    $stmt = $pdo->prepare("SELECT * FROM subcategoria"); 
    // Executa a consulta preparada.
    $stmt->execute(); 
    // Busca todos os resultados da consulta e os armazena no array $subcategorias como um array associativo.
    $subcategorias = $stmt->fetchAll(PDO::FETCH_ASSOC); 
    
} catch (PDOException $e) {
    // Em caso de erro, exibe uma mensagem de erro em vermelho.
    echo "<p style='color:red;'>Erro ao listar subcategorias: " . $e->getMessage() . "</p>";
}
?>
...
<head>
    ...
    function confirmDeletion() {
        return confirm('Tem certeza que deseja deletar esta subcategoria?'); /* Alerta de confirmação. */
    }
    </script>
...
<body>
    ...
    <table>
        ...
        <tr>
            <td><?php echo $subcateg['id_sub']; ?></td>
            <td><?php echo $subcateg['id_categoria']; ?></td>
            <td><?php echo $subcateg['nome']; ?></td>
            
            <td>
                <a href="editar_subcategorias.php?id_sub=<?php echo $subcateg['id_sub']; ?>" class="action-btn">Editar</a>
                <a href="excluir_subcategorias.php?id=<?php echo $subcateg['id_sub']; ?>" class="action-btn delete-btn" onclick="return confirmDeletion();">Excluir</a>
            </td>
        </tr>
        <?php endforeach; ?>
    </table>
    <div class="botoes-linha">
        <a href="exportar_excel.php?tipo=subcategoria" target="_blank" class="botao-link">Exportar Subcategorias</a>
        <a href="cadastrar_subcategorias.php" class="botao-link">Cadastrar Nova Subcategoria</a>
